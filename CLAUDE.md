...
| `telegram-bot/src/commands/notifyClient.ts` | 4-step wizard: (secret prompt or admin bypass) → validate → show inline keyboard of last 5 pending jobs from the past 7 days (no `status` field) → select job → Success/Failed buttons → update `status`/`completedAt`/`completedBy` in DB → send outcome message to original requestor |
...
**`package-lock.json` scan for uninstalled optional and peer deps** — two categories of packages are never placed in `node_modules` by `npm install` yet still need to be downloaded: (1) platform-specific optional deps (`@esbuild/win32-x64`, `@esbuild/darwin-arm64`, etc.) that npm skips because they don't match the host OS/CPU, and (2) optional peer deps of installed packages (e.g. `@mui/material-pigment-css` declared via `peerDependenciesMeta.optional: true` in `@mui/material`) that npm skips because the consuming project didn't include them. npm v7+ (lockfileVersion 2/3) writes all such packages into `package-lock.json` — platform-specific optional deps with `optional: true`, peer deps with `peer: true` (and sometimes also `optional: true`). After `walkNodeModules`, `resolver.ts` reads `package-lock.json` and adds every entry carrying `optional: true` OR `peer: true` that is not already in `seen`. Entries with more than one `node_modules/` path segment are skipped (those are nested deps of transitive packages, which the walk already handles).
