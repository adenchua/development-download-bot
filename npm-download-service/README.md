...
3. **Resolve** — merges `dependencies`, `devDependencies`, and `peerDependencies` (resolving complex peer dep version ranges to concrete versions via `semver`), writes a merged `package.json` to a temp directory, runs `npm install --ignore-scripts` to materialise the full dependency tree, then runs three passes to collect packages npm skipped installing: **(a)** reads `package-lock.json` for platform-specific optional deps that don't match the host OS/CPU (e.g. `@esbuild/win32-x64`); **(b)** scans each installed package's `package.json` for optional peer deps (e.g. `@mui/material-pigment-css` from `@mui/material`, `esbuild` from `vite`) that npm v11 no longer records in the lockfile; **(c)** calls `npm view <name>@<version> optionalDependencies` for each package added in (b) to capture its own optional deps (e.g. all 27 `@esbuild/*` platform variants when `esbuild` itself was only an optional peer dep and never installed)
